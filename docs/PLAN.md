...
| D2 | Source of `Test-GhAuthScope` | Extract and generalize the `gh auth status`-parsing preflight from `PS-MCS/gh-org/temp-migration/Invoke-PSMCSPSScriptAnalyzerMigration.ps1` | Same repo already proved the pattern against real `gh auth status` output shapes. |
