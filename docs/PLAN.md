...
| D6 | Publishing | v0.1.0 = git tag only, no PSGallery push. On tag push, `.github/workflows/release.yml` (added in PR-E) creates a GitHub Release with auto-generated notes and a downloadable `.zip` artifact (staged module compressed by the `CreateBuildArtifact` PSake task). Consumer repos install via `Install-Module -Path <checkout>` OR by extracting the release .zip. | Defers a `PSGALLERY_API_KEY` secret and prerelease-tag workflow until real consumer demand exists. GitHub Release is not a PSGallery publish; it just gives consumers a stable download URL and readable changelog view. |
