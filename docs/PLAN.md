...
| D2 | Source of `Test-GhAuthScope` | Extract and generalize the `gh auth status`-parsing preflight from `PS-MCS/gh-org`. **NOTE (2026-07-01 preflight):** the exact path from issue #1 (`temp-migration/Invoke-PSMCSPSScriptAnalyzerMigration.ps1`) returns HTTP 404 — likely renamed or removed after that migration finished. Locate the current path in `PS-MCS/gh-org` (search for `gh auth status` string) before starting PR-D. | Same repo already proved the pattern against real `gh auth status` output shapes. |
